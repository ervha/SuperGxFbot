...
module.exports = {
	...
	async autocomplete(interaction) {
		...
		const choices = bosyuData.map(b => ({
			name: `${b.questOption}`.slice(0, 100),
			value: b.messageId
		}));
	...
	async execute(interaction) {
		...
		try {
			...
			try {
				...
				targetMsg = await channel.messages.fetch(currentBosyu.messageId);
				const embed = targetMsg.embeds[0];
				if (embed) {
					const authorField = embed.fields.find(field => field.name === '募集者');
					if (authorField) {
						isAuthor = authorField.value.includes(`<@${interaction.user.id}>`);
					}
				}
			} catch (e) {
				await interaction.reply({ content: '募集メッセージの取得に失敗しました。', flags: MessageFlags.SuppressNotifications, flags: MessageFlags.Ephemeral });
				return;
			}
			...
			if (targetMsg && targetMsg.embeds[0]) {
				try {
					...
					const fields = targetMsg.embeds[0].fields.map(f => {
